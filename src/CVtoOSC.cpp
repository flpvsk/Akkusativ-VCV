...
struct CVtoOSC : Module {
  ...
  void fromJson(json_t *rootJ) override {
    ...
    if (address1J)
      address1 = json_string_value(address1J);
    isAddress1Dirty = true;
  }
  ...
  void onUrlUpdate(const std::string &newUrl) {
...
struct URLTextField : ui::TextField {
  ...
  void drawLayer(const DrawArgs &args, int layer) override {
    ...
    if (this == APP->event->hoveredWidget)
      state = BND_HOVER;
    ...
    bndOutlineBox(args.vg,
                  ...
                  cr[2],
                  cr[3],
                  bndTransparent(textFieldTheme.outlineColor));
    if (state != BND_ACTIVE) {
      end = -1;
    }
    ...
    bndIconLabelCaret(args.vg, 32, 0., box.size.x, box.size.y - 16.f, -1,
...
struct URLDisplay : LedDisplay {
  void setModule(CVtoOSC *module) {
    auto *textField = createWidget<URLTextField>(Vec(0, 0));
...
struct AddressTextField : LedDisplayTextField {
  ...
  void step() override {
    ...
    if (!module || !module->isAddress1Dirty)
      return;
  ...
  void onChange(const ChangeEvent &e) override {
    if (module)
      module->address1 = getText();
  }
};

struct AddressDisplay : LedDisplay {
  void setModule(CVtoOSC *module) {
    auto *textField = createWidget<AddressTextField>(Vec(0, 0));
    textField->box.size = box.size;
    textField->multiline = false;
    textField->module = module;
    addChild(textField);
...
struct CVtoOSCWidget : ModuleWidget {
  explicit CVtoOSCWidget(CVtoOSC *module) {
    setModule(module);
    setPanel(
    ...
    auto *urlDisplay = createWidget<URLDisplay>(Vec(0, h += 52));
    ...
    auto *address1Display = createWidget<AddressDisplay>(Vec(0, h += 32));
